...
winplot <- function(combo){
  ...
  for(i in c(1:10)){
    h$x[i] <- .1*i-.05
    temp <- filter(combo, result > .1*(i-1), result < .1*i)
...
l4 <- glm(Win~Tcon+Bcon+Twin+Bwin+Tloss+Bloss+Trpi+Brpi+Tbpi_o+Bbpi_o+Tken_o+Bken_o+Tken_n+Bken_n
          ...
          +T3+T4+T5+T6+T7+T8, data=aa, family = "binomial")
a$result <- suppressWarnings(predict(l4, a, type = "response"))
winplot(a[c(1:192),])
...
for(i in c(1:224)){
  if(a$result[i] > .5){
    ...
    a$ML[i] <- a$result[i]/(1-a$result[i])*-100
    a$Winpct[i] <- a$result[i]
  }
  if(a$result[i] < .5){
    ...
    a$ML[i] <- (1-a$result[i])/a$result[i]*100
    a$Winpct[i] <- 1-a$result[i]
  }
}
...
l14 <- glm(Win~Tcon+Bcon+Twin+Bwin+Tloss+Bloss+Tbpi_o+Bbpi_o+Tbpi_n+Bbpi_n+Toff+Boff+Tdef+Bdef+Tsos
           +Bsos+Tw50+Bw50+Tl50+Bl50+Tw150+Bw150+Tl150+Bl150+This+Bhis, data=cc, family = "binomial")
c$result <- suppressWarnings(predict(l14, c, type = "response"))
winplot(c[c(1:96),])
...
for(i in c(1:112)){
  if(c$result[i] > .5){
    ...
    c$ML[i] <- c$result[i]/(1-c$result[i])*-100
    c$Winpct[i] <- c$result[i]
  }
  if(c$result[i] < .5){
    ...
    c$ML[i] <- (1-c$result[i])/c$result[i]*100
    c$Winpct[i] <- 1-c$result[i]
  }
}
...
l23 <- glm(Win~Tcon+Bcon+Tloss+Bloss+Trpi+Brpi+Toff+Boff+Tdef+Bdef+Tsos+Bsos+T12+B12+Tw50+Bw50+Tl50
           +Bl50+Tw150+Bw150+Tl150+Bl150+Tstr+Bstr+This+Bhis, data=ff, family = "binomial")
e$result <- suppressWarnings(predict(l23, e, type = "response"))
#winplot(e[c(1:48),])
...
for(i in c(1:56)){
  if(e$result[i] > .5){
    ...
    e$ML[i] <- e$result[i]/(1-e$result[i])*-100
    e$Winpct[i] <- e$result[i]
  }
  if(e$result[i] < .5){
